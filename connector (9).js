    ...
    return t.card("customFieldItems", "id").then(function (card) {
      ...
      return t.board("customFields").then(function (board) {
        ...
        if (isBirthdayToday(birthdateText)) {
          console.log("🎂 MATCH! Moving card to top and showing badge");

          // Move the card to the top of its current list
          t.getRestApi().then(function(restApi) {
            restApi.put("/cards/" + card.id + "/pos", { value: "top" })
              .then(function() {
                console.log("🎂 Card moved to top successfully");
              })
              .catch(function(err) {
                console.log("🎂 Could not move card:", err);
              });
          }).catch(function(err) {
            console.log("🎂 REST API not available:", err);
          });
